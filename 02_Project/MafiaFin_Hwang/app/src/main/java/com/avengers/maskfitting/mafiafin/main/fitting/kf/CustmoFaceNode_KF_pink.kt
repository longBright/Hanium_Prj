package com.avengers.maskfitting.mafiafin.main.fitting.kf
...
class CustomFaceNodeKF_pink(augmentedFace: AugmentedFace?,
                     val context: Context
    ...
    override fun onActivate() {
        ...
        ViewRenderable.builder()
            .setView(context, R.layout.activity_img)
            .build()
            .thenAccept { uiRenderable: ViewRenderable ->
                uiRenderable.isShadowCaster = false
                uiRenderable.isShadowReceiver = false
                ...
                uiRenderable.view.findViewById<ImageView>(R.id.activity_img).setImageResource(R.drawable.kf_pink)
            }
            .exceptionally { throwable: Throwable? ->
                throw AssertionError(
                    "Could not create ui element",
    ...
    override fun onUpdate(frameTime: FrameTime?) {
        ...
        augmentedFace?.let {face ->
            ...
            getRegionPose(FaceRegion.MUSTACHE)?.let {
                mustacheNode?.localPosition = Vector3(it.x, it.y - 0.035f, it.z + 0.018f)
                mustacheNode?.localScale = Vector3(0.115f, 0.105f, 0.075f)
            }
        }
    }
}